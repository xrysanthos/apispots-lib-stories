...
var SwaggerApiDefinition = function (_ApiDefinition) {
  _inherits(SwaggerApiDefinition, _ApiDefinition);

  function SwaggerApiDefinition() {
    _classCallCheck(this, SwaggerApiDefinition);

    return _possibleConstructorReturn(this, (SwaggerApiDefinition.__proto__ || Object.getPrototypeOf(SwaggerApiDefinition)).apply(this, arguments));
  }

  _createClass(SwaggerApiDefinition, [{
    key: 'getDefinition',
    ...
    value: function getDefinition(id) {
      ...
      if (_lodash2.default.isEmpty(this.spec.definitions) || typeof this.spec.definitions[id] === 'undefined') {
        return null;
      }

      // get the original spec definition
      var definition = _lodash2.default.cloneDeep(this.spec.definitions[id]);
      ...
      if (_lodash2.default.isEmpty(definition.properties)) {
        ...
        _lodash2.default.each(definition.properties, function (o, key) {
          ...
          if (typeof o.$$ref !== 'undefined') {
            ...
            var _id = o.$$ref.replace('#/definitions/', '');
            o.type = _id;
          }
          ...
          if (o.type === 'array') {
            if (!_lodash2.default.isEmpty(o.items) && !_lodash2.default.isEmpty(o.items.$$ref)) {
              var _id2 = o.items.$$ref.replace('#/definitions/', '');
              o.items.type = _id2;
            }
          }
    ...
    value: function path(id) {
      ...
      _lodash2.default.each(definition, function (o) {
        ...
        _lodash2.default.each(o.parameters, function (p) {
          if (p.in === 'body') {

            if (typeof p.schema !== 'undefined' && typeof p.schema.$$ref !== 'undefined') {
              var schema = p.schema.$$ref.replace('#/definitions/', '');
              p.schema = schema;
              p.hasSchema = true;
            } else if (typeof p.schema !== 'undefined' && typeof p.schema.items !== 'undefined' && typeof p.schema.items.$$ref !== 'undefined') {
              var _schema = p.schema.items.$$ref.replace('#/definitions/', '');
        ...
        o.responses = _lodash2.default.map(o.responses, function (o, key) {
          ...
          if (typeof o.schema !== 'undefined') {
            ...
            } else if (_lodash2.default.has(o.schema, '$$ref')) {
    ...
    key: 'getOperationResponseDescription',
     ...
     * @return {[type]}        [description]
     */
    value: function getOperationResponseDescription(opId, status) {

      var description = '';

      try {

        var operation = this.getOperation(opId);

        // check if the responses section
        // has such a status entry
        if (_lodash2.default.has(operation.responses, status)) {
    ...
    key: 'getResponseSchemaDefinition',
    value: function getResponseSchemaDefinition(opId, status) {
      ...
      try {
        ...
        if (!_lodash2.default.isEmpty(response.schema)) {
          obj.type = response.schema.type;

          // is this a collection of objects?
          if (typeof response.schema.items !== 'undefined' && typeof response.schema.items.$$ref !== 'undefined') {
            obj.schema = response.schema.items.$$ref.replace('#/definitions/', '');
          } else if (_lodash2.default.has(response.schema, '$$ref')) {
    ...
    get: function get() {
      ...
      var schemas = _lodash2.default.chain(_lodash2.default.cloneDeep(this.spec.definitions)).map(function (o, key) {
        o.name = key;
        return o;
      }).sortBy('name').value();
    ...
    get: function get() {
      ...
      var definitions = _lodash2.default.cloneDeep(this.spec.securityDefinitions);
...
exports.default = SwaggerApiDefinition;